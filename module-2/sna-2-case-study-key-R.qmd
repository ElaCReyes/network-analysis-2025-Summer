...
pacman::p_load(ggplot2, tidyverse, readr, igraph, tidygraph, ggraph)

# library(janitor) # for data wranglig
# library(tidyverse)
# library(igraph)
# library(tidygraph)
# library(ggraph)
...
ggraph(dlt1_network, #network object
       layout = "fr") + #Fruchterman-Reingold (FR) layout 
  geom_edge_link(alpha = .2) + #change the opacity
  geom_node_point(aes(color = role1, #change node charateristics. Based on variable role1
                      size = local_size())) + #degree node
  theme_graph()
...
**The likelihood of an edge to be mutual in a directed network.**
...
ggraph(dlt1_network) +
  geom_node_point(aes(size = in_degree)) + #use in degree for node size
  geom_edge_link(aes(color = reciprocated), # change edge color based on reciprocity variable
                 alpha = .2) + #changing the transparency of the edge
  theme_graph()
```
