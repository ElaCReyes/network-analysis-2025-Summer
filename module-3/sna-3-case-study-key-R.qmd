...
```{r load-packages}
#pacman::p_load(ggplot2, tidyverse, readr, igraph, tidygraph, ggraph, janitor, readxl)

#YOUR CODE HERE
library(tidyverse)
library(tidygraph)
library(ggraph)
library(skimr) #package to make summaries
library(igraph)
```
...
```{r unnest-tokens}
ties_2 <- ties_1 %>%
  unnest_tokens(input = target, #have in mind that target means reaceiver
                output = receiver,
                to_lower = FALSE) %>%
...
```{r save-edges}
# DO NOT RUN AGAIN
# Update: last saved on July 17th, 2025 in Day 4 of LASER
...
ccss_network |> 
  as_tibble() |>
  skim() #similar to summarize() provides summar stats
```
...
# 1add row numbers to both data frames to match them up
ccss_tweets_2 <- ccss_tweets %>% 
  mutate(row_id = row_number())

vader_ccss_2 <- vader_ccss %>% 
  mutate(row_id = row_number())


# Join the sentiment scores with the original tweets
ccss_with_sentiment <- ccss_tweets_2 %>%
  left_join(vader_ccss_2, by = "row_id")

# Identify the "haters" - users with negative sentiment
haters <- ccss_with_sentiment %>%
  filter(compound <= -0.05) %>%  # negative sentiment threshold
  group_by(screen_name) %>%      # assuming screen_name is the username column
  summarise(
    negative_tweets = n(),
    avg_negative_score = mean(compound)
  ) %>%
  arrange(avg_negative_score)     # most negative first
